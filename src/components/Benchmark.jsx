...
const Benchmark = () => {
  ...
  // Get actual exam data for comparison
  const { getExamCount, getDaysToShow } = useChartsExport(actualData || [], {
    searchTerm: '',
    statusFilter: '',
    employeeFilter: '',
    showGold: false,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-900">Phân tích & Dự báo</h1>
      ...
      <div className="overflow-hidden">
        <div className="px-6 py-4 flex justify-between items-center">
          <h2 className="text-lg font-semibold text-gray-900">So sánh Thực tế vs Định mức</h2>
          <div className="text-sm text-gray-600">Phân tích hiệu suất 7 ngày gần nhất</div>
        </div>
              ...
              {performanceData?.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <div className="text-sm font-semibold text-gray-900">{item.avgActualCasesPerDay}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <div className="text-sm text-gray-900">
                      {item.so_ca_ngay_bs_min === item.so_ca_ngay_bs_max 
                        ? `${item.so_ca_ngay_bs_min}`
                        : `${item.so_ca_ngay_bs_min}-${item.so_ca_ngay_bs_max}`
                      }
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      ...
                      {item.status === 'overload' ? '🔴 Quá tải' : 
                       item.status === 'underload' ? '⚪ Dư thừa' : '🟢 Tối ưu'}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
      ...
      <div className="overflow-hidden">
        <div className="px-6 py-4 flex justify-between items-center">
          ...
          <div className="text-sm text-gray-600">Thông số kỹ thuật cho từng chuyên khoa</div>
        </div>
