...
const BenchmarkECGChart = ({ 
  ...
  const chartData = useMemo(() => {
    ...
    data.forEach(item => {
      ...
      // Add ECG counts to each examination date
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold text-gray-900">Điện tâm đồ</h3>
          ...
          <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <Line
              ...
              dot={(props) => {
                ...
                const maxDays = chartData.filter(d => d.ecg === maxValue && maxValue > 0)
                
                let shouldHighlight = false
                if (maxDays.length > 0) {
                  if (maxDays.length === 1) {
                    // Only one max day, highlight it
                    shouldHighlight = payload?.ecg === maxValue
                  } else {
...
export default BenchmarkECGChart
